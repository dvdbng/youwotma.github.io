...
* Dance.
*
* Full license: https://bengoarocandio.com/html5tweets/COPYING
* Chicken Dance instructions: https://bengoarocandio.com/html5tweets/DANCE
*/
...
var searchTerm = location.hash || "#MichaelJackson";
...
function insertarOrdenado(twt){
    ...
    if(!ins) avaliableTweets.push(twt);
    avaliableTweetsUpdated();
}

function loadTweets(){
    avaliableTweetsLastUpdated = +new Date();
    // Changed to a local snapshooot because now twitter requires authentication to use the search endpoint
    $.getJSON('./demo.json', function(data){
        var res = data.statuses;
        for(var i = 0; i<res.length;++i){
            ...
            img.src = res[i].user.profile_image_url;
        }
    });
}
...
function updateInnerCanvas(){
    for(var i = 0, l=alphaAnimating.length; i<l; ++i){
        ...
        if(hld.tweet) renderTweet(hld.tweet,0,0,opacity);
        ictx.restore();
    }
}
...
function renderTweet(tweet,x,y,op){
    ...
    ictx.fillStyle = "#ccc";
    ictx.font = "20px sans";
    ictx.fillText(prettyDate(tweet["created_at"]) + " by @" + tweet.user.screen_name, x+80 ,y+30+(vpos+0.5)*padded_font_size);
